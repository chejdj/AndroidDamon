...
class RemoteService : Service() {
    ...
    inner class MyBinder : RemoteChat.Stub() {
        override fun getProcessName(): String {
            Log.d(TAG, "remote service  process name :" + processName)
            return processName
        }
    }
    ...
    inner class MyServiceConnection : ServiceConnection {
        override fun onServiceConnected(name: ComponentName?, service: IBinder?) {
            ...
            Log.d(TAG, "local service process name is:" + (service as RemoteChat).processName)
        }

        override fun onServiceDisconnected(name: ComponentName?) {
            Log.d(TAG, "local service disconnect!!")
            val intent = Intent(this@RemoteService, LocalService::class.java)
            this@RemoteService.startService(intent)
            this@RemoteService.bindService(intent, myServiceConnection, Context.BIND_IMPORTANT)
        }
    }
}